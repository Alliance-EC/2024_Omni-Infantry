...
void ServeoMotorControl()
{
    ...
    for (size_t i = 0; i < servo_idx; i++) {
        if (servo_motor_instance[i]) {
            ...
            // 比较值=自动重装载值*（初始高电平时间+目标角度对应重载值占比*）；
            switch (Servo_Motor->Servo_type) {
                case Servo180:
                    if (Servo_Motor->Servo_Angle_Type == init_mode)
                        compare_value[i] = 20000 * 0.5 / 20 + (Servo_Motor->Servo_Angle.Init_angle / 90 / 20) * 20000;
                    if (Servo_Motor->Servo_Angle_Type == end_mode)
                        compare_value[i] = 0.5 * 20000 / 20 + Servo_Motor->Servo_Angle.Final_angle * 20000 / 20 / 90;
                    if (Servo_Motor->Servo_Angle_Type == free_mode)
